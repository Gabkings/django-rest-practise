...
class JournalistVC(APIView):

    def get(self, request):
        news = Journalist.objects.all()
        serializer = JournalistSerializer(news, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class JournalistDetailsModify(APIView):
    def get_object(self, pk):
        news = get_object_or_404(Journalist, pk=pk)
        return news

    def get(self, request, pk):
        news = self.get_object(pk)
        serializer = JournalistSerializer(news)
        return Response(serializer.data, status=status.HTTP_200_OK)
